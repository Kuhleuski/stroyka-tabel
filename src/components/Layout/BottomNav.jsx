...
export function BottomNav({ currentPage, onNavigate }) {
    ...
    // Бургер иконка (крестик при открытии)
    const BurgerIcon = () => (
    ...
    // Обычное состояние — 4 иконки + бургер
    if (!isExpanded) {
        return (
    ...
    // Расширенное состояние — дополнительные пункты поверх основных
    return (
        <div className={styles.bottomNav}>
            <div className={styles.navContainer}>
                {/* Основные пункты остаются, но становятся прозрачными/неактивными */}
                <div className={`${styles.mainGroup} ${styles.mainGroupDimmed}`}>
                    {mainItems.map(({ key, icon }) => (
                        <button
                            key={key}
                            className={`${styles.navItem} ${currentPage === key ? styles.active : ''}`}
                ...
                <button 
                    className={`${styles.burgerBtn} ${styles.burgerActive}`}
                    onClick={toggleMenu}
                >
                    <span className={styles.navIcon}><BurgerIcon /></span>
                </button>
            ...
            {/* Дополнительные пункты — выезжают снизу, перекрывая основные */}
